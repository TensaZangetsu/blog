                ...
                <div class="main wrapper clearfix row">
                    <article class="column column-9">
                        <article class="single-article">
                            <header>
                                <?php
    //Calling post class read method which will then do the query and return the result
    //Result will be put trogh while loop and echo table data in apropriate tags.
    $posts = new Posts($db);
$stmt = $posts->read();
while($row_post = $stmt->fetch_assoc()){
                    ?>
                                    ...
                                    <a class="read-more" href="/post.php?id=<?php echo $row_post['posts_id'] ?>" title="Read More">Read More</a>
